...
class TypeClsDataset(Dataset):
    def __init__(self,
                 ...
                 predict=False,
                 eval_type="test"):
        super(TypeClsDataset, self).__init__()
        ...
        self.contexts = samples['context']
        self.label = samples["type"]
        self.position = samples["position"]


    def __getitem__(self, idx):
        text = self.contexts[idx]
        type_label = self.label[idx]
        ...
        # TODO: generate decoder input_ids for T5
        return {
            "input_ids":
            ...
            torch.tensor(np.array(token_type_ids, dtype=np.int64),
                         dtype=torch.long),
            "position":
            torch.tensor(np.array(position, dtype=np.int64),
                         dtype=torch.long),
            "type_label":
            torch.tensor(np.array(type_label, dtype=np.int64),
                            dtype=torch.long),
        }
...
class KgProjectDataset(Dataset):
    def __init__(self,
                 samples,
                 data_processor,
                 tokenizer,
                 ...
                 predict=False,
                 eval_type="test"):
        super(KgProjectDataset, self).__init__()
        ...
        self.contexts = samples['context']
        self.answers = samples["answer"]
        self.answer_contexts = samples["answer_context"]


    def __getitem__(self, idx):
        text = self.contexts[idx]
        answers = self.answers[idx]
        answer_context = self.answer_contexts[idx]
        inputs = self.tokenizer.encode_plus(text,
                                            max_length=self.max_length,
                                            padding='max_length',
                                            truncation=True)
        ...
        outputs = self.tokenizer.encode_plus(answer_context,
                                    max_length=self.max_length,
                                    padding='max_length',
                                    truncation=True)
        ...
        return {
            ...
            torch.tensor(np.array(attention_mask, dtype=np.int64),
                         dtype=torch.long),
            # "decoder_input_ids":
            # torch.tensor(np.array(decoder_input_ids, dtype=np.int64),
                            # dtype=torch.long),
            # "decoder_attention_mask":
            # torch.tensor(np.array(decoder_attention_mask, dtype=np.int64),
            #                 dtype=torch.long), 
            "labels":
            torch.tensor(np.array(label_ids, dtype=np.int64),
                            dtype=torch.long),
        }
